...
export class MovieService {
  ...
  public getAccountInfo(): Observable<AccountDetails> {
    return this.httpClient.get<AccountDetails>(`${this.baseUrl}/account/null`)
      .pipe(catchError(err => of('error', err)));
  }

  public getLists(accountId: string): Observable<any> {
    return this.httpClient.get<any>(`${this.baseUrl}/account/${accountId}/lists`)
      .pipe(catchError(err => of('error', err)));
  }

  public getWatchList<Type>(accountId: number): Observable<Type> {
    return this.httpClient.get<Type>(`${this.baseUrl}/account/${accountId}/watchlist/movies`)
      .pipe(catchError(err => of('error', err)));
  }

  public getFavorites<Type>(accountId: number): Observable<Type> {
    return this.httpClient.get<Type>(`${this.baseUrl}/account/${accountId}/favorite/movies`)
      .pipe(catchError(err => of('error', err)));
  }
  ...
  public getMovieDetails<Type>(id: number): Observable<Type> {
    return this.httpClient.get<Type>(`${this.baseUrl}/movie/${id}`)
      .pipe(catchError(err => of('error', err)));
  }
  ...
  public getNowPlaying<Type>(page: number = 1, lang: string = 'en-US'): Observable<Type> {
    return this.httpClient.get<Type>(`${this.baseUrl}/movie/now_playing?page=${page}&language=${lang}`)
      .pipe(catchError(err => of('error', err)));
  }

  public getPopular<Type>(page: number = 1, lang: string = 'en-US'): Observable<Type> {
    return this.httpClient.get<Type>(`${this.baseUrl}/movie/popular?page=${page}&language=${lang}`)
      .pipe(catchError(err => of('error', err)));
  }

  public getTopRated<Type>(page: number = 1, lang: string = 'en-US'): Observable<Type> {
    return this.httpClient.get<Type>(`${this.baseUrl}/movie/top_rated?page=${page}&language=${lang}`)
      .pipe(catchError(err => of('error', err)));
  }

  public getUpcoming<Type>(page: number = 1, lang: string = 'en-US'): Observable<Type> {
    return this.httpClient.get<Type>(`${this.baseUrl}/movie/upcoming?page=${page}&language=${lang}`)
      .pipe(catchError(err => of('error', err)));
  }

  public addToFavorite<Type>(mediaId: number, account_id: number): Observable<Type> {
  ...
  public removeFromFavorite<Type>(mediaId: number, account_id: number): Observable<Type> {
